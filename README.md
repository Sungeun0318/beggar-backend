...
- 카카오 Local API의 키워드/주소/좌표 변환 요청은 추천 응답 지연을 줄이기 위해 2초 타임아웃을 적용한다.
...
- `mapUrl` 검색어는 주소 전체가 아니라 `업소명 + 짧은 지역`으로 생성해 카카오맵에서 장소명 검색이 우선되게 한다.
...
- [ ] 영수증 등록 완료 후 방별 거지력 점수 재계산 — 착한가격업소 인증 방문 수 반영
