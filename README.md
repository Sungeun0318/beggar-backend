...
- `GET /rooms/{no}/recommend?tag={tag}&region={region}` — 착한가격업소 API 호출 → 지역/태그/가격 기준 정렬 → `RecommendationResponse` 반환
- 응답 `places[]`는 업소명, 업종, 최저 메뉴 가격, 주소, 카테고리 기본 이미지 경로(`thumbnailUrl`), 카카오맵 검색 링크(`mapUrl`)를 포함한다.
- 현재 지역은 쿼리 파라미터로 받는다. 방 지역 저장은 `rooms` 설정 API 구현 시 DB 컬럼/DTO와 함께 확정한다.
- **추천 이력 저장 테이블 없음**. 영수증 매칭을 정확히 하려면 후속 작업에서 추천 이력 테이블을 추가한다.
...
- 착한가격업소 매칭 성공 시 `good_price_*`, `good_price_matched`, `good_price_verified_at` 업데이트
...
- [ ] 추천 이력 저장 테이블 + 영수증 등록 시 추천 업소 매칭
