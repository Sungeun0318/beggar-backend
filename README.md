...
- `POST /rooms` — 방 생성 (room_name, room_code 생성, `is_friends`(친구 전용/자유방), tags[] 동시에 `room_purpose_tags` INSERT)
- `GET /rooms/{no}` — 방 상세 (+ tags, + 예산 결과)
- `GET /rooms/open` — **자유방 둘러보기** (`is_friends = FALSE` 방 목록, 페이징·카테고리 필터)
- `POST /rooms/join` — `room_code`로 `room_members` INSERT (`status='ACTIVE'`) — 친구 전용 방용
- `POST /rooms/{no}/join` — **자유방 직접 입장** (코드 불필요, `is_friends = FALSE`인 방만 허용)
- `GET /rooms/{no}/members` — 입장 현황 (초대 화면 폴링). **금액 절대 노출 금지**, `EXISTS(budgets)` 여부만 반환

> `is_friends` 정책 (rooms 컬럼): `TRUE` = 친구 전용 (초대 코드/링크로만 입장) / `FALSE` = 자유방 (자유방 목록 노출 + 코드 없이 입장 가능). 익명 예산·거지력 산정은 양쪽 동일 규칙.
